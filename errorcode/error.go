...
func MQPubError(subject string, data []byte, err error) *Error {
	return &Error{
		Code: MQ_Pub_Error,
...
func MQSubError(subject string, err error) *Error {
	return &Error{
		Code: MQ_Sub_Error,
		Err:  errors.Errorf("subject:%v err:%v", subject, err.Error()),
	}
}
