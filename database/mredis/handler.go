...
func (h *Handler) Get(ctx context.Context, key string) (string, error) {
	return h.rdsCmdable.Get(ctx, key).Result()
}
...
func (h *Handler) UpdateZsetMember(ctx context.Context, key string, score float64, member string) error {
	...
	err = h.rdsCmdable.ZAdd(ctx, key, redis.Z{Score: score, Member: member}).Err()
	if err != nil {
		return err
	}
	return nil
...
func (h *Handler) DelZsetMember(ctx context.Context, key string, member []string) error {
	return h.rdsCmdable.ZRem(ctx, key, member).Err()
}

// 上鎖
func (h *Handler) Lock(ctx context.Context, key string, second int) error {
	success, err := h.rdsCmdable.SetNX(ctx, key, "", time.Duration(second*int(time.Second))).Result()
	if !success && err == nil {
